...
-- The table movie contains 7997 rows

SELECT COUNT(*)
FROM genre;

-- The table genre contains 14662 rows

SELECT COUNT(*)
FROM director_mapping;

-- The table director_mapping contains 3867 rows

SELECT COUNT(*)
FROM role_mapping;

-- The table role_mapping contains 15615 rows

SELECT COUNT(*)
FROM names;

-- The table names contains 25735 rows

SELECT COUNT(*)
FROM ratings;

-- The table ratings contains 7997 rows
...
SELECT 
    ...
    -- SUM(CASE WHEN year IS NULL THEN 1 ELSE 0) AS year_null,
	...
	SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS null_production 
FROM movie;
...
-- First part
SELECT year as Year, SUM(id) as number_of_movies
FROM movies
GROUP BY Year
...
WITH total AS
(
	SELECT movie_id, COUNT(genre) AS num
	FROM genre
	GROUP BY movie_id
    HAVING num=1
)
SELECT COUNT(movie_id) AS movies_with_unique_genre
FROM total;
...
SELECT g.genre, ROUND(SUM(m.duration)/count(m.duration)) as avg_duration
FROM movie m
INNER JOIN genre g
ON m.id = g.movie_id
GROUP BY g.genre
...
SELECT g.genre, COUNT(g.genre) as movie_count, RANK() OVER(ORDER BY COUNT(g.movie_id) DESC) as genre_rank
FROM movie m
INNER JOIN genre g
ON m.id = g.movie_id
GROUP BY g.genre;
...
WHERE r.total_votes > 1000 AND m.country = 'USA' AND MONTH(date_published) = 3 AND year = 2017
GROUP BY g.genre
ORDER BY movie_count DESC;
...
SELECT title, avg_rating, g.genre AS genre
from movie m
INNER JOIN ratings r
ON r.movie_id = m.id
INNER JOIN genre g
...
SELECT 
	SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS name_nulls, 
...
SELECT genre,
	...
	SUM(ROUND(AVG(duration),2)) OVER(ORDER BY genre ROWS 5 PRECEDING) AS moving_avg_duration
FROM movie m 
INNER JOIN genre g 
ON m.id= g.movie_id
GROUP BY genre
...
SELECT DISTINCT genre, m.year AS year, title AS movie_name, worlwide_gross_income, 
...
SELECT n.name, SUM(total_votes) AS total_votes, COUNT(rm.movie_id) AS movie_count, avg_rating,
...
--------------------------------------------------------------------------------------------*/
-- Type you code below:
SELECT n.id as director_id, n.name as director_name, avg_rating, SUM(total_votes),
	MIN(avg_rating) AS min_rating, MAX(avg_rating) as max_rating, duration as total_duration
FROM director_mapping d
INNER JOIN movie m
ON d.movie_id = m.id
INNER JOIN names n
...
GROUP BY n.id, avg_rating, duration;
